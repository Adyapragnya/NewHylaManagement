...
const getIconForZoom = (zoom) => {
  ...
  if (zoom > 9.75) return { width:8, height: 14, type: 'large' };
  if (zoom > 8.75) return { width: 8, height: 12, type: 'large'  };
  ...
  return { width: 7, height: 7, type: 'point' };
};
      ...
      const updateMarkers = () => {
        ...
        if (markerClusterGroupRef.current) {
          ...
          markerClusterGroupRef.current = L.markerClusterGroup({
            maxClusterRadius: 60, // Adjust this value to change cluster size
          });
